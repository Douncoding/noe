...
public class PetRegisterPresenter extends BasePresenter<PetRegisterContract.View> implements  PetRegisterContract.Presenter {
    ...
    public void onSubmit(Pet pet) {
        ...
        // 기본정보확인
        if (pet.getName().isEmpty() || pet.getBirthday().isEmpty() || pet.getNickname().isEmpty()) {
        ...
        photoRef.child(timestamp + ".jpg").putBytes(outputStream.toByteArray()).addOnSuccessListener(taskSnapshot -> {
            pet.setPictureUrl(taskSnapshot.getDownloadUrl().toString());

            DatabaseReference petsRef = FirebaseUtil.getPetsRef();
            DatabaseReference beaconRef = FirebaseUtil.getBeaconRef();
            DatabaseReference peopleRef = FirebaseUtil.getCurrentUserHasPetRef();

            Map<String, Object> updateUserData = new HashMap<>();
            final String petKey = petsRef.push().getKey();
            // 반려동물 기록
            updateUserData.put(FirebaseUtil.getPath(petsRef) + "/" + petKey, pet);
            // 비콘 기록
            updateUserData.put(FirebaseUtil.getPath(beaconRef) + "/" + beaconRef.push().getKey(), beacon);
            // 소유자 기록
            updateUserData.put(FirebaseUtil.getPath(peopleRef) + "/" + petKey, true);
