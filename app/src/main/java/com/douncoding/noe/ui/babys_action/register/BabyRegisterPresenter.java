...
import com.douncoding.noe.ui.pets_action.register.PetRegisterContract;
...
public class BabyRegisterPresenter extends BasePresenter<BabyRegisterContract.View> implements  BabyRegisterContract.Presenter {
     ...
     */
    @Override
    public void onSubmit(Baby baby) {
        // 이미지 등록 확인
        ...
        // 기본정보확인
        if (baby.getName().isEmpty() || baby.getBirthday().isEmpty() || baby.getNickname().isEmpty()) {
        ...
        photoRef.child(timestamp + ".jpg").putBytes(outputStream.toByteArray()).addOnSuccessListener(taskSnapshot -> {
            baby.setPictureUrl(taskSnapshot.getDownloadUrl().toString());

            DatabaseReference babyRef = FirebaseUtil.getBabysRef();
            DatabaseReference beaconRef = FirebaseUtil.getBeaconRef();
            DatabaseReference peopleRef = FirebaseUtil.getCurrentUserHasBabyRef();

            Map<String, Object> updateUserData = new HashMap<>();
            final String babyKey = babyRef.push().getKey();
            // 자녀 기록
            updateUserData.put(FirebaseUtil.getPath(babyRef) + "/" + babyKey, baby);
            // 비콘 기록
